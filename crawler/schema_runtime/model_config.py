...
def _load_openclaw_model_config() -> dict[str, Any]:
    ...
    if config:
        return config
    raise ValueError(
        "OpenClaw Gateway token not found. Set OPENCLAW_GATEWAY_TOKEN or configure gateway.auth.token in openclaw.json."
    )
...
def _read_openclaw_token_from_config() -> str:
    config_path = Path(os.environ.get("OPENCLAW_CONFIG_PATH", "")).expanduser() if os.environ.get("OPENCLAW_CONFIG_PATH") else Path.home() / ".openclaw" / "openclaw.json"
